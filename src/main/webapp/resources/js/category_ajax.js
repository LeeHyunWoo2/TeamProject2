$(document).ready(function () {
    ...
    window.addEventListener('popstate', function (event) {
        if (event.state) {
            ...
            updateCategoryUI(category); // UI를 카테고리에 맞게 업데이트
            loadBooksByCategory(category); // 비동기 요청으로 데이터를 다시 로드
        }
    });
    // 초기화 및 카테고리 로드
    loadCategories();
    ...
    function updateCategoryInURL(category) {
        ...
        if (currentCategory !== category){
            ...
            updateURLParam('category', category, false);
        }

        updateURLParam('category', category); // `category`로 통일
    }

    function loadBooksByCategory(category) {
        loadPage(1, undefined, '', category); // `loadPage`에서 `category`를 사용하도록 수정
    }
    ...
    function updateCategoryUI(category){
        // 현재 URI의 카테고리에 맞게 UI 업데이트하는 로직 (뒤로가기 앞으로가기에도 리스트가 갱신되어 보이게끔)
        updateMenuTitle(category ? `category: ${category}` : 'All categories');
    }
});
